...
st.title("📊 CGPA Calculator (Levels + Semesters + HTML Export)")
...
def input_courses(semester_id):
    ...
    for i in range(num):
        ...
        with col2:
            grade = st.number_input(f"{semester_id} - G.P for {course or f'Course {i+1}'}", min_value=0, max_value=5.0, step=1, key=f"{semester_id}_grade_{i}")
        with col3:
            credit = st.number_input(f"{semester_id} - Unit for {course or f'Course {i+1}'}", min_value=0, max_value=5.0, step=1, key=f"{semester_id}_credit_{i}")
